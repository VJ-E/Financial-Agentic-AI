    ...
    return (
        <div className="p-4 md:p-8 space-y-6 pb-24 max-w-4xl mx-auto">
            ...
            {rawProfile?.activeGroupId && (
                <div className="inline-block bg-black text-white px-3 py-1 font-bold uppercase text-xs">
                    Group Mode Active
                </div>
            )}
            
            {/* Timeframe Toggle */}
            <div className="flex border-4 border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
                {['day', 'week', 'month', 'overall'].map((tf) => (
                    <button
                        ...
                        className={`flex-1 py-3 font-black uppercase text-xs sm:text-sm border-r-4 border-black last:border-r-0 transition-colors ${
                            timeframe === tf ? 'bg-[#008CD4] text-white shadow-[inset_0px_4px_0px_0px_rgba(0,0,0,0.2)]' : 'hover:bg-gray-100 text-black'
                        }`}
                    >
                        {tf}
                    </button>
            ...
            {/* Chart */}
            <div className="bg-white border-4 border-black p-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] h-[400px]">
                <h3 className="font-black uppercase mb-6 text-xl border-b-4 border-black pb-2 inline-block">Cashflow Comparison</h3>
                <ResponsiveContainer width="100%" height="80%">
                    <BarChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }} barSize={60}>
                        <CartesianGrid stroke="#000" vertical={false} strokeWidth={2} />
                        <XAxis dataKey="name" stroke="#000" tick={{ fill: '#000', fontWeight: '900', fontFamily: 'monospace', fontSize: 14 }} axisLine={false} tickLine={false} dy={10} />
                        <YAxis stroke="#000" tick={{ fill: '#000', fontWeight: '900', fontFamily: 'monospace' }} axisLine={false} tickLine={false} tickFormatter={(val) => `₹${val/1000}k`} dx={-10} />
                        <Tooltip 
                            cursor={{ fill: '#008CD4', opacity: 0.1 }}
                            contentStyle={{ backgroundColor: '#fff', border: '4px solid #000', borderRadius: 0, boxShadow: '6px 6px 0px 0px rgba(0,0,0,1)', fontWeight: '900', color: '#000' }}
                            formatter={(value: number) => [formatCurrency(value), "Amount"]}
                        />
                        <Bar dataKey="amount" fill="#008CD4" stroke="#000" strokeWidth={4} radius={0} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
