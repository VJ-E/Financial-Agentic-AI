...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                const errMsg = Array.isArray(data.detail) ? data.detail.map((err: any) => `${err.loc?.[1] || 'Field'}: ${err.msg}`).join(", ") : (data.detail || "Login failed.");
                setError(errMsg);
                return;
            }
            ...
            <div className="relative z-10 w-full max-w-md p-4">
                ...
                <form onSubmit={handleLogin} className="border-8 border-black bg-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] p-8 space-y-6">
                    ...
                    <p className="text-center font-bold text-sm uppercase">
                        No account?{" "}
                        <a href="/signup" className="underline underline-offset-4 decoration-4 hover:bg-[#008CD4] hover:text-white px-1 transition-colors">
                            Sign Up
                        </a>
                    </p>
                </form>
