...
PLAYBOOKS = {
    "CORE_SKILL": [get_financial_data, add_transaction, delete_transaction, update_transaction],
    "GOAL_SKILL": [get_financial_data, create_goal, fund_goal, delete_goal],
...
async def router_node(state: AgentState):
    ...
    elif any(k in lower_msg for k in ["goal", "save for", "vault", "target"]):
...
def chatbot(state: AgentState):
    ...
    for key in valid_keys:
        all_runnables.append(ChatGroq(api_key=key, model="llama-3.3-70b-versatile", temperature=0, max_retries=1).bind_tools(skill_tools))
